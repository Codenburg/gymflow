...
The component MUST consume `searchParams` via Next.js 16 RSC idioms (e.g. `await searchParams`). The readers MUST NOT use the `"use cache"` directive, MUST NOT use `use cache: private`, and MUST NOT import the singleton `prisma` instance inside a `"use cache"` body.
