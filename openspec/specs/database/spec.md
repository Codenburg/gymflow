...
(Previously: `getStats` was cached via `unstable_cache` with NO tag at all — a latent bug, since the mutation actions had no way to invalidate it. The result was that the dashboard's stat counts could stay stale indefinitely after a routine mutation. The tag was added in v0.18.0 and preserved through the v0.19.0 `use cache` migration.)
