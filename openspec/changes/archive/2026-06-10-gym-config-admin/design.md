...
- **[ ] Structured `horario` instead of free-text** — the current `horario: String?` field lets the admin write anything ("8:00 a 22:00", "Lun-Vie 7-22hs", "Abierto de 8 a 22", etc.), which produces inconsistent render output across deploys and no validation. The user explicitly asked for a structured form per day of the week (Lun, Mar, ..., Dom), each with `abierto: boolean` + `apertura: time | null` + `cierre: time | null`, so the **application** controls the render format (not the admin's free-text). The data model should change to either:
  - `horarioJson: Json?` storing `{ lunes: {abierto, apertura, cierre}, martes: {…}, …, domingo: {…} }`, or
  - a normalized `HorarioDia` child table with `(gymId, diaSemana, abierto, apertura, cierre)` and a unique constraint on `(gymId, diaSemana)`.
  The form (`GymConfigManager` → Schedule sub-form) needs 7 day cards instead of the current single textarea. The `HoursSection` component on `/informacion` renders a uniform string from the structure (e.g. "Lun a Vie 8:00 a 22:00 · Sáb 9:00 a 14:00 · Dom cerrado"). This is a follow-up because the current free-text approach is functional and unblocking — but the structural fix is small and isolated enough to do as its own change.
...
- [x] **`horario` shape**: free-text confirmed in spec, but should we offer a structured fallback (open/close time per day) in v2? **Resolved (deferred)** — promoted to Known Follow-ups (Structured `horario` instead of free-text). Will be done in a follow-up SDD change.
